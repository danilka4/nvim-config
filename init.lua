...
vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
        ...
        vim.keymap.set({ 'n', 'x' }, 'gq', '<cmd>lua vim.lsp.buf.format()<cr>', opts)
    end,
})
vim.diagnostic.config({
    ...
    float = {
        border = "single", -- or "rounded", "double", or nil for no border
        -- source = "always", -- or "if_many"
        -- header = "heheheha",   -- Customize header text
        -- format = function(diagnostic)
        --     -- Customize the format of the diagnostic message in the float
